...
from cfg import cfg_sam_point_box_test
# from precpt import run_precpt
from models.discriminator import Discriminator

# from siren_pytorch import SirenNet, SirenWrapper

args = cfg_sam_point_box_test.parse_args()
...
def Uentropy(logits,c):
    ...
    pc = F.softmax(logits,dim=0)  # 1 4 240 240 155
    # logpc = F.log_softmax(logits, dim=1)  # 1 4 240 240 155
    logpc = F.log_softmax(logits,dim=0)
    # u_all1 = -pc * logpc / c
    u_all = - pc * logpc / math.log(c)
    # max_u = torch.max(u_all)
    # min_u = torch.min(u_all)
    # NU = torch.sum(u_all, dim=1)
    # NU = torch.sum(u_all,dim=0)
    return u_all

def make_sub_folder(output_dir,item_path,sub_list):
    subfolder_path_list = []
     # 在样本文件夹下创建子文件夹
...
def make_sub_folder_2(output_dir,sub_list):
    subfolder_path_list = []
...
def get_network(args, net, use_gpu=True, gpu_device = 0, distribution = True):
    ...
    if net == 'sam':
        ...
        from models.MedSAM.segment_anything import SamPredictor,sam_model_registry
        from models.MedSAM.segment_anything.utils.transforms import ResizeLongestSide
        options = ['vit_b']
        if args.encoder not in options:
            raise ValueError("Invalid encoder option. Please choose from: {}".format(options))
        else:
            net = sam_model_registry[args.encoder](args,checkpoint=args.medsam_ckpt).to(device)
...
def get_network_pa_sam(args, net, use_gpu=True, gpu_device = 0, distribution = True):
    """ return given network
    """

    if net == 'sam':
        from model_pa_sam.segment_anything_training.build_sam import sam_model_registry
        from model_pa_sam.segment_anything_training.utils.transforms import ResizeLongestSide
        options = ['default','vit_b','vit_l','vit_h']
        if args.encoder not in options:
            raise ValueError("Invalid encoder option. Please choose from: {}".format(options))
        else:
            net = sam_model_registry[args.encoder](checkpoint=args.sam_ckpt).to(device)
    elif net =='medsam':
        from models2.MedSAM.segment_anything.build_sam import sam_model_registry
        from models2.MedSAM.segment_anything.utils.transforms import ResizeLongestSide
        options = ['vit_b']
        if args.encoder not in options:
            raise ValueError("Invalid encoder option. Please choose from: {}".format(options))
        else:
            net = sam_model_registry[args.encoder](checkpoint=args.medsam_ckpt).to(device)
...
def get_network_sam(args, net, use_gpu=True, gpu_device = 0, distribution = True):
    """ return given network
    """

    if net == 'sam':
        from models2.sam import SamPredictor, sam_model_registry
        # from models2.sam.utils.transforms import ResizeLongestSid
        options = ['default','vit_b','vit_l','vit_h']
        if args.encoder not in options:
            raise ValueError("Invalid encoder option. Please choose from: {}".format(options))
        else:
        ...
        from models2.MedSAM.segment_anything import SamPredictor,sam_model_registry
        # from models2.MedSAM.segment_anything.utils.transforms import ResizeLongestSide
        options = ['vit_b']
        if args.encoder not in options:
            raise ValueError("Invalid encoder option. Please choose from: {}".format(options))
...
# def vis_image_isic_point(imgs, pred_masks, gt_masks, save_path, dice,iou,reverse = False, points = None,box=None):

#     imgs_1 = imgs.cpu().numpy().squeeze(0).transpose(1,2,0)
#     pred = pred_masks.cpu().detach().numpy().squeeze(0).transpose(1,2,0)
#     gt = gt_masks.cpu().numpy().squeeze(0).transpose(1,2,0)
#     dice_1 = dice
#     iou_1 = iou


#     pred = np.where(pred >= 0.5, True, False)
...
#     y1_1,x1_1 =points[0][0]

#     # cv2.imwrite(os.path.join(save_path[0], 'prompt.png'), imgs_2)
#     # cv2.imwrite(os.path.join(save_path[1], 'gt.png'), gt)
#     # cv2.imwrite(os.path.join(save_path[2], 'pred_dice_{}_iou_.png'.format(dice_1,iou_1)), pred)

#     sub_folder_path = make_sub_folder_2(save_path,['prompt','gt','mask'])


#     # image_copy = tensor_to_cv2_image(imgs)
...
#     cv2.imwrite(os.path.join(sub_folder_path[0], 'prompt.png'), imgs_2)

#     cv2.imwrite(os.path.join(sub_folder_path[1], 'gt.png'), gt)
#     cv2.imwrite(os.path.join(sub_folder_path[2], 'pred_dice_{}_iou_.png'.format(dice_1,iou_1)), pred)
...
#     #     # ave_mask = np.mean(pred_masks, axis=0).squeeze(0)

#     #     #  iter0_mask = np.where(np.mean(pred_list1, axis=0)>=0,1,0)
#     #         # ave_mask = np.where(ave_mask >= 0.5 * (np.max(ave_mask)), 1, 0)
#     #         # ave_mask = np.where(ave_mask >= 0.5, 1, 0)
#     # # 二值化 pred_masks 和 gt_masks
#     # pred_masks = (pred_masks >= 0.5).float()  # 将预测掩码二值化
#     # gt_masks = (gt_masks >= 0.5).float()  # 将真实掩码二值化
#     # gt_masks = gt_masks[:, 0, :, :].unsqueeze(1).expand(b, 3, h, w)  # 扩展维度用于可视化

#     # # 将图像转换为 uint8 范围 [0, 255] 并绘制边界框
#     # imgs_uint8 = (imgs[:row_num] * 255).to(torch.uint8)


#     # if box is not None:
#     #     if box.dim() == 2 and box.size(0) == 1:
#     #         box = box.repeat(row_num, 1, 1)  # 如果只有一个框，则复制为 `row_num` 个

#     #     if box.dim() == 3:
#     #         box = box[:row_num, :, :]
#     #     else:
#     #         box = box[:row_num, :].unsqueeze(1)  # 确保 box 的维度为 (row_num, 1, 4)

#     #     for i in range(row_num):
#     #         imgs_uint8[i] = draw_bounding_boxes(imgs_uint8[i], box[i], colors="red", width=2)

#     # if points is not None:
#     #     for i in range(row_num):
#     #         ps = points[i]
#     #         ps = ps.to(dtype=torch.int)
#     #         for p in ps:
#     #             x, y = p[0].item(), p[1].item()
#     #             # 在 gt_masks 和 imgs_uint8 上绘制点
#     #             for dx in range(-5, 6):
#     #                 for dy in range(-5, 6):
#     #                     if 0 <= x+dx < w and 0 <= y+dy < h:
#     #                         gt_masks[i, :, y+dy, x+dx] = torch.tensor([0.5, 0.1, 0.4]).to(gt_masks.device)
#     #                         imgs_uint8[i, :, y+dy, x+dx] = torch.tensor([255, 0, 0]).to(imgs_uint8.device)

#     # # 将 imgs_uint8 转换回浮点数并将值调整到 [0, 1] 范围
#     # imgs_float = imgs_uint8.float() / 255.0

#     # # 扩展 pred_masks 的维度用于可视化
#     # pred_masks = pred_masks[:, 0, :, :].unsqueeze(1).expand(b, 3, h, w)

#     # tup = (imgs_float, pred_masks[:row_num, :, :, :], gt_masks[:row_num, :, :, :])
...
def vis_image_isic_png_point(imgs, pred_masks, gt_masks, save_path, dice,iou,reverse = False, points = None):
    ...
    cv2.circle(imgs_2,(int(x1_1.item()),int(y1_1.item())),3,(255,0,0),-1)
    # cv2.rectangle(imgs_2, (x1.cpu().item(), y1.cpu().item()), (x2.cpu().item(), y2.cpu().item()), (0,0,255), 1)  # 2 表示线的粗细
    # cv2.circle(imgs_2,(points[0][0][0].item(),points[0][0][1].item()),3,(0,255,0),-1)
    cv2.imwrite(os.path.join(sub_folder_path[0], 'prompt.png'), imgs_2)
...
def vis_image_isic_png_3point(imgs, pred_masks, gt_masks, save_path, dice,iou,reverse = False, points = None):
...
def vis_image_isic_png(imgs, pred_masks, gt_masks, save_path, dice,iou,reverse = False, points = None,box=None):
    for i in range(1):

        imgs_1 = imgs[i].cpu().numpy().transpose(1,2,0)
        pred = pred_masks[i].cpu().detach().numpy().transpose(1,2,0)
        gt = gt_masks[i].cpu().numpy().transpose(1,2,0)
        dice_1 = dice
        iou_1 = iou
...
def vis_image_isic_box(imgs, pred_masks, gt_masks, save_path, dice,iou,reverse = False, points = None,box=None):
...
def vis_image(imgs, pred_masks, gt_masks, save_path, reverse = False, points = None,box=None):
    ...
    if c == 2: # for REFUGE multi mask output
        ...
        for i in range(0, c):
            ...
            gts.append(gt)
        ave_mask = np.mean(preds, axis=0).squeeze(0)

        # #  iter0_mask = np.where(np.mean(pred_list1, axis=0)>=0,1,0)
        #     # ave_mask = np.where(ave_mask >= 0.5 * (np.max(ave_mask)), 1, 0)
        #     # ave_mask = np.where(ave_mask >= 0.5, 1, 0)
        # ave_mask = np.where(ave_mask >= 0, 1, 0)
        ...
        #  iter0_mask = np.where(np.mean(pred_list1, axis=0)>=0,1,0)
            # ave_mask = np.where(ave_mask >= 0.5 * (np.max(ave_mask)), 1, 0)
            # ave_mask = np.where(ave_mask >= 0.5, 1, 0)
        # ave_mask = np.where(pred_masks >= 0, 1, 0)
        ...
        # if box is not None:
            ...
            # image_copy = tensor_to_cv2_image(imgs)
            # cv2.rectangle(image_copy, (x1.cpu().item(), y1.cpu().item()), (x2.cpu().item(), y2.cpu().item()), (0,0,255), 1)  # 2 表示线的粗细
        ...
        # tup = (imgs_uint8, pred_masks[:row_num, :, :, :], gt_masks[:row_num, :, :, :])
        imgs_float = imgs_uint8.float() / 255.0
...
def tensor_to_cv2_image(tensor):
    """
    Converts a PyTorch tensor image to a format readable by OpenCV.
    
    Args:
        tensor (torch.Tensor): The tensor image to convert, expected shape (C, H, W).
        
    Returns:
        numpy.ndarray: The OpenCV-readable image with shape (H, W, C) and dtype uint8.
    """
    # Ensure the tensor is on CPU
    # 转换 tensor 为 numpy 数组
    np_img = tensor.cpu().numpy().squeeze(0)

    # 将通道顺序从 (C, H, W) 转换为 (H, W, C)
    np_img = np.transpose(np_img, (1, 2, 0))

    # 如果图像的值在 [0, 1] 范围内，则缩放到 [0, 255] 并转换为 uint8
    if np_img.max() <= 1.0:
...
def eval_seg_train(pred,true_mask_p,threshold):
    '''
...
def eval_seg(pred_final,true_mask_p,threshold):
    '''
    threshold: a int or a tuple of int
    masks: [b,2,h,w]
    pred: [b,2,h,w]
    '''
    b, c, h, w = pred_final.size()
    if c == 2:
        iou_d, iou_c, disc_dice, cup_dice = 0,0,0,0
        for th in threshold:

            gt_vmask_p = (true_mask_p > th).float()
            vpred = (pred_final > th).float()
            vpred_cpu = vpred.cpu()
            disc_pred = vpred_cpu[:,0,:,:].numpy().astype('int32')
            cup_pred = vpred_cpu[:,1,:,:].numpy().astype('int32')
        ...
        eiou, edice,eece,esm,efm = 0,0,0,0,0
        for th in threshold:
            ...
            eece += ece_binary(disc_pred,disc_mask)
            esm += smeasure(disc_pred,disc_mask)
            efm +=Fmeasure_calu(disc_pred,disc_mask,0.5)
            
        return eiou / len(threshold), edice / len(threshold),eece /len(threshold),esm/len(threshold),efm /len(threshold)
...
def random_click(mask, point_labels = 1):
    ...
    # 生成随机偏移的点并确保它在前景区域内
    def is_valid_point(point, foreground_pixels):
        return tuple(point) in map(tuple, foreground_pixels)

    # i = 0
    while True:
        random_offset = np.random.randint(-offset_range, offset_range + 1, size=2)
        random_point = point + random_offset
        # i+=1
        # print(i)
        if is_valid_point(random_point, indices):
            y,x = random_point
            ...
            return point_labels,random_point

    # return point_labels, indices[np.random.randint(len(indices))]


def random_click_3point(mask, point_labels = 1):
    offset_range=5
    # check if all masks are black
    max_label = max(set(mask.flatten()))
    # if max_label == 0:
    ...
    while len(selected_points) <10 :
        ...
        # 确保生成的点在前景区域内，并且是唯一的
        if tuple(random_point) in map(tuple, indices) and tuple(random_point) not in selected_points:
            selected_points.append(tuple(random_point))
    ...
    return point_labels, selected_points
