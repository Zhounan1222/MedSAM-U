...
if args.dataset == 'isic_2017':
    ...
    nice_test_loader = DataLoader(isic_test_dataset, batch_size=args.b, shuffle=False, num_workers=8, pin_memory=True)
    '''end'''
if args.dataset == 'ISIC_2017_point&box':
    '''isic data'''
    isic_train_dataset = ISIC2017_pointbox(args, args.data_path, transform = transform_train, transform_msk= transform_train_seg, mode = 'Train')
    isic_test_dataset = ISIC2017_pointbox(args, args.data_path, transform = transform_test, transform_msk= transform_test_seg, mode = 'Test')
...
if args.dataset == 'isic_2017_box':
    '''isic data'''
    isic_train_dataset = ISIC2017_box(args, args.data_path, transform = transform_train, transform_msk= transform_train_seg, mode = 'Train')
    isic_test_dataset = ISIC2017_box(args, args.data_path, transform = transform_test, transform_msk= transform_test_seg, mode = 'Test')
    ...
    '''REFUGE data'''
    refuge_train_dataset = REFUGE(args, args.data_path, transform = transform_train, transform_msk= transform_train_seg, mode = 'Train')
...
tol, (eiou, edice) = function_medsam.test_medsamu(args, nice_test_loader, start_epoch, net)
